...
export default function RestaurentDetails() {
  ...
  const fetchRestaurent = async () => {
    try {
      const response = await axios.get(`${IP_ADDRESS}/restaurents/${id}`);
      setRestaurent(response.data);
    } catch (error) {
      console.log("Error fetching restaurent:", error);
    }
  };
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.imageWrapper}>
        <Image source={{ uri: restaurent.image_url }} style={styles.image} />
      ...
      <View style={styles.infoSection}>
        <Text style={styles.title}>{restaurent.restaurent_name}</Text>
        <TouchableOpacity
          style={styles.row}
        ...
        >
          ...
          <Text style={styles.ratingText}>
            {restaurent.avg_rating} <Text style={styles.ratingCount}>({restaurent.rating_count} reviews)</Text>
          </Text>
        </TouchableOpacity>
        <View style={styles.row}>
          <Ionicons name="location" size={18} color="#1a974e" />
          <Text style={styles.locationText}>{restaurent.address}</Text>
        </View>
        <View style={styles.row}>
          <Ionicons name="pricetag" size={18} color="#1a974e" />
          <Text style={styles.priceText}>Starting from ${restaurent.price}</Text>
        </View>
...
<FlatList
  ...
  renderItem={({ item }) => (
    ...
    >
      <Image source={{ uri: item.image_url || restaurent.image_url }} style={styles.menuImage} />
